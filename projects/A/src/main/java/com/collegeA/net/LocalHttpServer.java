...
public class LocalHttpServer {
    ...
    public static void startServer() {
        try {
            ...
            // 接口4：获取本院统计信息（供集成服务器聚合）
            server.createContext("/api/local/statistics", new HttpHandler() {
                @Override
                public void handle(HttpExchange exchange) throws IOException {
                    try (Connection conn = DatabaseConnection.getConnection()) {
                        if (conn == null) {
                            writeXml(exchange, 500, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><error>DB connection is null</error>");
                            return;
                        }
                        ...
                        rs = conn.createStatement().executeQuery("SELECT COUNT(*) FROM StudentA");
                        if (rs.next()) studentCount = rs.getInt(1);
                        rs.close();

                        rs = conn.createStatement().executeQuery("SELECT COUNT(*) FROM CourseA");
                        if (rs.next()) courseCount = rs.getInt(1);
                        rs.close();

                        rs = conn.createStatement().executeQuery("SELECT COUNT(*) FROM CourseChoiceA");
                        if (rs.next()) choiceCount = rs.getInt(1);
                        rs.close();

                        String xmlResp = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                                + "<statistics><college name=\"A\">"
