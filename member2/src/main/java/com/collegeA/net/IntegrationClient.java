...
public class IntegrationClient {
    ...
    // 从集成服务器请求跨院共享课程（XML字符串形式）
    public static String getSharedCoursesFromOtherColleges() {
        try {
            URL url = new URL(INTEGRATION_SERVER_URL + "/api/integration/sharedCourses?source=A");
    ...
    public static String sendEnrollOrDropRequest(String xmlPayload) {
        try {
            URL url = new URL(INTEGRATION_SERVER_URL + "/api/integration/courseChoice?source=A");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
